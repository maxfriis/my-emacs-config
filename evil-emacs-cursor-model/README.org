...
#+end_src
The package is also available in the non-GNU repository.
...
When you use the mode, you might find yourself only using the (evil-paste-before) operation to paste.
Vim's need for two paste operations is compensation for the normal-state cursor model.
Other editors only have one paste operation and this mode also mainly have one.
(evil-paste-after) is still bound to =P= and can be useful, but you can contextually replace it with =jp= or =lp=.

Last I swap evil-mode's =o= and =O= so they are consistent with =p= and =P= when you paste lines.

** Summary

These are all normal-state keybindings:

| This mode | Vim | Alternative       | Lisp function       |
|-----------+-----+-------------------+---------------------|
...
Besides these operations the mode manipulate motions so the cursor between characters model will work.
...
When operations change as described above, you might start to adopt a new philosophy.
The idea is to work with motions among line nuggets.

With this mode /operations/ focus on the current line and you can replace all layers with motions.
...
In most situations you will probably solve whatever you want to do, with roughly the same number of keystrokes.
...
Ultimately the modal framework focus on editing text (working with the current line) rather than adding new text (working with additional lines).
...
** Combined operation and motion

Vim has operations that combine a functionality with a motion.
A useful application of the operation/motion behavior is Vim's =xp= which /swap/ two characters and move the cursor forward.
This is less accessible in this mode because I swap the paste operations (=xP=).
...
In practice =C-v= is still usable and useful but it does have bugs tied to motions and the end of line position.

Emacs' =C-x <SPC>= (rectangle-mark-mode) works in insert-state but not in normal-state.
If =C-v= fails I suggest you fall back on marking with =v= and use the =C-x r= bindings.

* Raison d'être speculation

The /line nugget/ mindset will use less operations and the logic is easier to learn and internalize.
