...
I use (set-face-attribute...) because stuff set by custom.el is hard to change for packages I might load in the future.
It is bad to set defaults that are hard to change (red text is hard to read on a red background).
