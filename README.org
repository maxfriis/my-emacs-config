...
Note that I don't use #73f (bad color contrast).
...
| #0f0  | normal   |   | #0ff  | replace |
| #00f  | motion   |   | #f0f  | emacs   |
|-------+----------+---+-------+---------|
Normal state is a green color.
Operator state is red to alert.
Insert state is the last yellow traffic light color and use a bar.
Replace state is cyan and use an hbar.
Motion and Emacs states have the remaining rgb colors.
...
I use my evil-cursor-between-mode to modify evil to use a cursor is between characters in normal state.
...
In my configuration the between characters behavior is isolated in evil-cursor-between-mode.el, which I have modified compared to [[https://www.dr-qubit.org/Evil_cursor_model.html][the author's original version]].
Standard evil behavior is easy to achieve by simply not loading that file or toggling the mode off.

You can find the mode on github at [[https://github.com/maxfriis/evil-cursor-between-mode][evil-cursor-between-mode]].
...
A few evil bindings specific to the change in cursor model are rebound inside evil-cursor-between-mode.el.
...
I often press =c= almost before I have figured out what motion I need.
...
The only thing left is to install nerd fonts with (nerd-icons-install-fonts) and restart.
