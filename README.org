...
|-------+----------+---+-------+---------|
| #f00  | operator |   | #0ff  | replace |
| #0f0  | normal   |   | #f0f  | emacs   |
| #00f  | motion   |   | #ff0  | insert  |
|-------+----------+---+-------+---------|
Normal state is a green color.
Operator state is red to alert.
Insert state is the last yellow traffic light color and has a bar cursor.
Replace state is cyan and has an hbar cursor.
...
I use my evil-emacs-cursor-model-mode to modify evil to use a cursor is between characters in normal state.
...
I also use HOLD combined with archive for canceled tasks.
