...
I have changed some keybindings and even modify the default evil-mode behavior with a mode I have composed.
The mode can be toggled with =<SPC> t e= but it can be confusing and it is on by default.
It is my configuration after all.

** Version requirement

Most of the configuration is fine with Emacs version 29.1, but my which-key
configuration requires version 30.1.  I think which-key was made native in that
version because I know the which-key package is much older.
...
I should probably use (deftheme...) and I will at some point.
...
I don't like or use custom.el for anything.
...
As root and sometimes on my laptop I use 8 ansi colors for the theme.
| Color | Use            |   | Color | Use             |
|-------+----------------+---+-------+-----------------|
...
|-------+----------------+---+-------+-----------------|
Operator state is automatically converted when shown on the red hl-line background.
...
Emacs usually provide what I need and I don't have to write functions.

* Package.el

I use package.el and (require 'foo) to install and load packages.
I used to use use-package but that seem to increase the number of garbage collections significantly (about 5 fold for me).
It is not clear to me that gains from the control (:defer) is worth that cost.

I think deferring also makes less sense combined with Emacs server/client and modern computers.
...
That means that I will set variables associated with a package right next to where I load it.
...
All this should make it fairly easy to convert to use-package or straight.
...
Standard evil behavior is easy to achieve by not loading the mode or simply toggling the mode off.
...
A clarifying note is added to this state when it's changed to HOLD.
I also use this state for canceled tasks which I archive.
...
3. Not all TODO items have or even should have a timestamp.
   ...
   Then I display other unscheduled items.
...
A few evil bindings specific to the change in cursor model are rebound inside evil-emacs-cursor-model-mode.el.
Besides that I choose to use Emacs' isearch and I have it bound to =s=.
evil-substitute can be mimicked with =cl=.
This comes natural to me since =c= is the most common way I enter insert state.
I often press =c= even before I have fully decided what motion I need to use.
